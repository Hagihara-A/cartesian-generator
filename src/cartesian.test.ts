...
test(`[{}, "1"], ["2", {a:""}, 3] => [{}, "2"]@0, ["1", {a:""}]@4, ["1", 3]@5`, () => {
  const prod = cartesian([{}, "1"], ["2", { a: "" }, 3]);
  expect.assertions(3);
  for (let i = 0; i < 6; i++) {
    const curr = prod.next();
    if (i === 0) expect(curr.value).toEqual([{}, "2"]);
    else if (i == 4) expect(curr.value).toEqual(["1", { a: "" }]);
    else if (i == 5) expect(curr.value).toEqual(["1", 3]);
  }
});
